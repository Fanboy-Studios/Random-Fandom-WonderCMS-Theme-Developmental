...
function alterAdmin($args) {
  ...
  # Place a label identifying these are Random Fandom theme options
  ...
  $themeLabel->nodeValue = "Rfmgtheme Theme Options";
  ...
  if (acGet('source') === 'config.txt') {
    ...
    $vals_html = "<p><ul>\n";
    $rfmgthemeOpts = getRfmgthemeOpts();
    foreach ($rfmgthemeOpts as $id => $label) {
  ...
  # Loop over the getRfmgthemeOpts() and made input fields for each one.
  $rfmgthemeOpts = getRfmgthemeOpts();
  foreach ($rfmgthemeOpts as $id => $label) {
    # Field label
    $real_id = preg_replace('/^theme:/', '', $id);
...
global $RfmgthemeConf;
$RfmgthemeConf = loadRfmgthemeConf();

# This function loads the global $RfmgthemeConf, preferentially from
# from config.txt if it exits, and if not, from wCMS settings that
# the theme adds.
function loadRfmgthemeConf() {
  global $Wcms;
...
# Gets an $RfmgthemeConf value or empty string if it does not exist
function acGet($key) {
  global $RfmgthemeConf;
  if (! (array_key_exists('loaded', $$RfmgthemeConf) and $RfmgthemeConf['loaded'])) {
    $$RfmgthemeConf = load$RfmgthemeConf();
  }
  if (array_key_exists($key, $$RfmgthemeConf)) {
    return $$RfmgthemeConf[$key];
  }
  return '';
}

# Returns the HTML for the footer's LinkedIn link.
# Uses $$RfmgthemeConf settings from theme's config.txt
...
# $Wcms->addListener('menu', 'getMenuRfmgtheme');
function getMenuRfmgtheme($args) {
  global $Wcms;
  $output = '';
